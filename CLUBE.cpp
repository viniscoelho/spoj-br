/*
	Resolucao:
		Calcula o numero maximo de tabuas usadas para forrar o salao
		Tanto na vertical quanto horizontal
		O maior numero de tabuas usadas e retornado se for possivel forrar o salao
*/
...
int main(){
    ...
    while ( cin >> M >> N && M && N ){
		...
		for ( int i = 0; i < 10010; i++ ) H[i] = V[i] = 0;
		for ( int i = 0; i < NT; i++ ){
			cin >> aux;
			H[aux]++;
			V[aux]++;
		...
		if ( (M*100) % L == 0 || (N*100)%L == 0 ){
			...
			if ( vertical != -1 ){
				if ( horizontal != -1 ) cout << min(vertical, horizontal) << "\n";
                else cout << vertical << "\n";
			}
			else if(horizontal != -1) cout << horizontal << "\n";
