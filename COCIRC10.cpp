...
{
	return (cmp(c.x, min(a.x, b.x)) >= 0 && cmp(c.x, max(a.x, b.x)) <= 0 && cmp(c.y, min(a.y, b.y)) >= 0 && cmp(c.y, max(a.y, b.y)) <= 0);
}
...
{
	if (!intersect(a, b, c, d, inter))
		return false; //segmentos paralelos
...
bool intersect(pt c1, double r1, pt c2, double r2, pt &p1, pt &p2)
{
	...
	if (cmp(d, r1 + r2) > 0 || cmp(d, r1 - r2) < 0)
		return false; //nao tem intersecao
	if (!cmp(d) && !cmp(r1, r2))
		return true; //sao iguais

	pt v = c2 - c1;
	p1 = c1 + v * r1 / v.length();
	if (!cmp(d - r1 - r2) || !cmp(d + r2 - r1))
	{
		p2 = p1;
		return true;
	} //tengencia interna/externa
...
int main()
{
	...
	while (scanf("%d", &n) && n)
	{
		...
		for (int i = 0; i < n; ++i)
			...
			{
				for (int k = j + 1; k < n; ++k)
					if (!colinear(circ[i], circ[j], circ[k]))
						points.pb(ccenter(circ[i], circ[j], circ[k]));
				sort(points.begin(), points.end());
				int m = 1;
				for (int r = 0; r < points.size(); ++r)
				{
					if (r + 1 < points.size() && points[r] == points[r + 1])
						m++;
					else
					{
						qtd = max(qtd, m);
