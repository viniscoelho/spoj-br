...
int dijkstra(int s, int t, list<is>::iterator son){
	...
	while ( !pq.empty() ){
		...
		if ( v == t ) return custo;
		if ( visited[v][atual.second[0]] == ok ) continue;
		visited[v][atual.second[0]] = ok;
		int ant = -1;
		for ( it = graph[v].begin(); it != graph[v].end(); ++it ){
			if ( atual.second[0] == it->second[0] ) continue;
			...
			else if ( ant == it->first ) pq.push( mp( mp( -(custo + int(it->second.size())), it->first ), it->second ) );
			ant = it->first;
		}
	}
	return INF;
...
int main(){
    ...
    while ( cin >> n && n ){
    	...
    	for ( int i = 0; i < n; ++i ){
			...
			if ( !ret.second ) de = bib[from];
			else{
				de = bib[from];
				t++;
			}
			ret = bib.insert( mp( to, t ) );
			if ( !ret.second ) para = bib[to];
			else{
				para = bib[to];
				t++;
			}
