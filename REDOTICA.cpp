...
void Link ( int x, int y ){
	if ( pset[x].rank > pset[y].rank ) pset[y].pai = x;
		...
		pset[x].pai = y;
		if ( pset[x].rank == pset[y].rank ) pset[y].rank = pset[y].rank+1;
	}
}
...
int findSet ( int x ){
	if ( pset[x].pai != x ) pset[x].pai = findSet ( pset[x].pai );
	return ( pset[x].pai );
}
...
bool menor ( const Taresta& a, const Taresta& b ){
	if ( a.peso < b.peso ) return true;
			...
			if ( a.para == b.para ) return ( a.de < b.de );
			else return ( a.para < b.para );
		else return false;
}
