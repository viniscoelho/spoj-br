...
int BFS(){
	REP( i, m ) if ( X1[i] && X2[i] ) return i;
	REP( i, m ) previ[i] = -1;
	queue<int> bfs;
	REP( i, m )
		if ( X1[i] ){
			bfs.push(i); visited[i] = ok;
		}
	while ( !bfs.empty() ){
		int v = bfs.front(); bfs.pop();
		REP( j, g[v].size() ){
			int u = g[v][j];
			if ( visited[u] == ok ) continue;
			previ[u] = v;
			if ( X2[u] ) return u;
			bfs.push(u); visited[u] = ok;
		}
	}
	return -1;
}
...
int matroid_intersection(){
	...
	for ( res = 0; true; res++ ){
		...
		REP( i, 1 )
			if ( base[i] ){
				inicializa();
				REP( j, m ) if ( base[j] && i != j ) unionSet( x[j], y[j] );
				REP( j, m ) if ( !base[j] && !isSameSet(x[j], y[j]) ) g[i].pb(j);
				REP( j, m ) if ( !base[j] ){
					maxcor[cor[i]]++;
		...
		inicializa();
		REP( i, m ) if ( base[i] ) unionSet( x[i], y[i] );
		REP( i, m ){
			X1[i] = X2[i] = false;
			if( !base[i] ){
				if ( !isSameSet(x[i], y[i]) ) X1[i] = true;
...
int main(){
	...
	while ( scanf("%d %d %d", &n, &m, &k) != EOF ) {
		...
		printf("Instancia %d\n", test++);
		if ( matroid_intersection() == n-1 ) printf("sim\n\n");
		else printf("nao\n\n");
	}
	return 0;
}
