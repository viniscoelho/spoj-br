...
{
    flag[node] = 0;
    if ( ld == rd ) tree[node] = segment(1, 0, 0);
    else
    {
        int tm = (ld + rd) >> 1;
        build(node*2, ld, tm);
...
{
    flag[node] %= 3;
    for ( int i = 0; i < flag[node]; i++ ) swap(&tree[node].h, &tree[node].e, &tree[node].r);
...
{
    if ( flag[node] ) lazy_propagate(node);
    return tree[node];
}
